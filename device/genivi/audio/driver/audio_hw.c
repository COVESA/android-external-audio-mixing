...
int create_socket(int *fd, const char* socket_name) {
  if ((*fd = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
    ALOGE("Cannot create datagram socket, errno: %d, %s", errno, strerror(errno));
    return -1;
  }
  ...
  if (bind(*fd, (struct sockaddr *)&addr,
    ...
    ALOGE("Bind error, errno: %d, %s", errno, strerror(errno));
    return -1;
  }

  ALOGD("Socket created successfully");
  return 0;
}
...
static void *out_write_worker(void * args)
{
    ...
    while (true) {
        ...
        sent = sendto(sockfd, buffer, pcm_frames_to_bytes(pcm, frames), 0, (struct sockaddr *)&serv_addr, serv_addr_len);
        if ( sent < 0 ) {
          ALOGE("PK>> sending socket ret value is %d, errno: %d: %s", sent, errno, strerror(errno));
        }
                                   ...
                                   const char *address __unused)
{
    ...
    if (ret == 0) {
        ...
        ALOGI("PK>> Output stream format %d, channel_mask %04x, sample_rate %u",
            config->format, config->channel_mask, config->sample_rate);
        ALOGI("PK>> Creating worker thread");
...
fallback_init(void)
{
    ...
    if (fptr != NULL) {
      ...
      if (c != EOF) {
          ...
          ALOGW("PK>> Genivi HAL for emulator");
          return;
      }
    }
